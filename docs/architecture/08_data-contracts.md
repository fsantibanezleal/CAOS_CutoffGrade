...
`data-pipeline/cglab/io/contract.py`. Declares the required schema (columns, units, ranges) + an explicit
...
`data-pipeline/cglab/core/{trace.py, manifest.py}`. Every run writes a compact trace (`cutoffgrade.trace/v1`) +
a manifest (`cutoffgrade.manifest/v2`) recording params, seed, engine+version, the artifact byte size, the measured
