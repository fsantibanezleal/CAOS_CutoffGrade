...
**Contract 1** (`data-pipeline/pipeline/io/contract.py`), and it lives in the **offline pipeline**. Be clear about
...
2. Validate it with Contract 1 from the pipeline venv: `pipeline.io.contract.validate_records([row])` (or
...
3. To see your deposit in the product, register it as a case: add the deck to
   `data-pipeline/pipeline/cases/lane_cases.py` and its mirror `frontend/src/lane/cases.ts` (a test keeps the two
