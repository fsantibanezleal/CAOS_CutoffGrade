...
export default function Benchmark() {
  ...
  return (
    <article className="page-body prose">
      <h1>Benchmark</h1>
      <p className="lede">{es ? 'Comparaciones cross-case, las que no dependen de un solo caso viven aquí (no en el App). Todas vienen del precálculo del motor TS.' : 'Cross-case comparisons, the ones that do not depend on a single case live here (not in the App). All come from the TS-engine bake.'}</p>
      ...
      {learned ? (
        ...
        <p className="cg-note">{es ? 'Modelos aprendidos no entrenados, regenerarlos con el paso de re-entrenamiento del precómputo. El optimizador exacto de Lane se ejecuta en vivo mientras tanto.' : 'Learned models not trained, regenerate them with the precompute retrain step. The exact Lane optimizer runs live meanwhile.'}</p>
      )}
