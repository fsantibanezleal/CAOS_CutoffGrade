          ...
          content: (
            <div className="cg-doc-sec">
              ...
              <Callout variant="note" title={es ? 'Dos lenguajes, un motor' : 'Two languages, one engine'}>
                {es ? 'El paso de horneado corre el MISMO TypeScript que el navegador (vía tsx) sobre los casos y emite el artefacto de resultados committeado. No hay un re-port de la economía a Python — el motor de Lane vive una sola vez, en TypeScript.' : 'The case-bake step runs the SAME TypeScript the browser runs (via tsx) over the cases and emits the committed results artifact. There is no Python re-port of the economics — the Lane engine lives once, in TypeScript.'}
              </Callout>
            </div>
          ),
        },
