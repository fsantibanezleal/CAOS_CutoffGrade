...
export default function Implementation() {
  ...
  return (
    <article className="page-body prose">
      ...
      <p className="lede">{es
        ? 'Una arquitectura de dos carriles: el motor económico de Lane es TypeScript y se ejecuta idéntico en ambos, vivo en el navegador y en el precómputo (Node). El carril de precómputo además entrena los dos modelos y precalcula los casos; dos contratos tipados unen ambos carriles, y un drift entre ellos rompe el chequeo de tipos.'
      ...
      <Tabs ariaLabel={es ? 'implementación' : 'implementation'} tabs={[
          ...
          content: (
            <div className="cg-doc-sec">
              <table className="cmp-table">
                <thead><tr><th>{es ? 'carril' : 'lane'}</th><th>{es ? 'qué' : 'what'}</th><th>{es ? 'se ejecuta en' : 'runs on'}</th></tr></thead>
                <tbody>
                  ...
                  <tr><td>{es ? 'Offline (precómputo)' : 'Offline (precompute)'}</td><td>{es ? 'el mismo motor TS ejecutado en Node + el entrenamiento de los dos modelos (torch → ONNX)' : 'the same TS engine run in Node + training the two models (torch → ONNX)'}</td><td>{es ? 'entorno de precómputo (torch)' : 'precompute environment (torch)'}</td></tr>
                  <tr><td>{es ? 'Replay (ligero)' : 'Replay (light)'}</td><td>{es ? 'reconstruye las trazas + manifests por caso desde los artefactos versionados' : 'rebuilds the per-case traces + manifests from the committed artifacts'}</td><td>numpy</td></tr>
                </tbody>
              </table>
              <p className="cg-cap">{es ? 'El gate de carril decide qué se ejecuta vivo: cliente ∧ runtimes ⊆ {motor-TS, onnxruntime-web} ∧ dentro de presupuesto → vivo; cualquier otra cosa cae al precómputo.' : 'The lane gate decides what runs live: client-side ∧ runtimes ⊆ {TS-engine, onnxruntime-web} ∧ within budget → live; anything else falls to precompute.'}</p>
            </div>
          ),
        },
        {
          ...
          content: (
            <div className="cg-doc-sec">
              <p><b>{es ? 'Contrato 1, el validador de depósito' : 'Contract 1, the deposit validator'}</b>{es ? ' (la compuerta de depósito propio): valida el depósito + la economía y rechaza o marca con una política de outliers explícita, de modo que un caso mal formado nunca llega al optimizador en silencio.' : ' (the "bring your own deposit" gate): validates the deposit + economics and rejects or flags with an explicit outlier policy, so a malformed case never reaches the optimizer silently.'}</p>
              ...
              <Callout variant="note" title={es ? 'Dos lenguajes, un motor' : 'Two languages, one engine'}>
                {es ? 'El paso de precálculo ejecuta el mismo TypeScript que el navegador (vía tsx) sobre los casos y emite el artefacto de resultados versionado. No hay un re-port de la economía a Python, el motor de Lane vive una sola vez, en TypeScript.' : 'The case-bake step runs the same TypeScript the browser runs (via tsx) over the cases and emits the committed results artifact. There is no Python re-port of the economics, the Lane engine lives once, in TypeScript.'}
              </Callout>
            </div>
          ),
        },
        {
          ...
          content: (
            <div className="cg-doc-sec">
              <p>{es
                ? 'El carril por defecto es ligero: el artefacto de resultados versionado es la salida real del motor TS, así que la integración continua y el replay nunca necesitan torch ni Node. Un comando de precómputo reconstruye las trazas + manifests por caso (sólo numpy); con la bandera de re-entrenamiento, re-precalcula los casos y entrena de nuevo los dos modelos (torch → ONNX). Todo es determinista: re-ejecutarlo produce un resultado byte-idéntico.'
              ...
              <table className="cmp-table">
                ...
                <tbody>
                  <tr><td>{es ? 'precálculo de casos' : 'case bake'}</td><td>{es ? 'depósito + deck → traza + manifest por caso' : 'deposit + deck → per-case trace + manifest'}</td><td>{es ? 'ligero (numpy)' : 'light (numpy)'}</td></tr>
